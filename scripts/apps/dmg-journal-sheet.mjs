 ...
 */
export default class DMGJournalSheet extends dnd5e.applications.journal.JournalSheet5e {
  constructor(doc, options) {
    super(doc, options);
    this.options.classes.push("dmg");
  }
  ...
  async _render(...args) {
    ...
    // Insert navigation
    const nav = this.document.getFlag("dnd5e_collection_2024", "navigation");
    if ( nav ) {
        ...
        <nav class="book-navigation">
          <ul>
            <li>${previous ? `<a class="content-link" data-uuid="${previous.uuid}" rel="prev"
              data-tooltip="DMG.JOURNAL.Previous" data-link data-tooltip-direction="LEFT">${previous.name}</a>` : ""}</li>
            <li>${up ? `<a class="content-link parent" data-uuid="${up.uuid}"
            data-link data-tooltip="DMG.JOURNAL.Up">${up.name}</a>` : ""}</li>
            <li>${next ? `<a class="content-link" data-uuid="${next.uuid}" rel="next"
              data-tooltip="DMG.JOURNAL.Next" data-link data-tooltip-direction="RIGHT">${next.name}</a>` : ""}</li>
          </ul>
        </nav>
      `);
    }
